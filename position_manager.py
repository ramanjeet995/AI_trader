...
def _find_protective_stop(symbol, trade_client):
    ...
    Bracket leg stops show as HELD (not OPEN) and live inside the parent
    order's 'legs' list — the parent itself shows as FILLED. We must query
    ALL orders with nested=True to find them.
    OTO stop children show as OPEN and appear directly.
    """
    try:
        # Check OPEN orders first (works for OTO stop children)
        ...
        for o in orders:
            if not hasattr(o, 'legs') or not o.legs:
                continue
            for leg in o.legs:
